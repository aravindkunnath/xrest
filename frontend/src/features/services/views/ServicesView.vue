...
import AddEndpointDialog from "@/features/dialogs/AddEndpointDialog.vue";
import SwaggerImportDialog from "@/features/dialogs/SwaggerImportDialog.vue";
import ShareRequestDialog from "@/features/dialogs/ShareRequestDialog.vue";
import UnsafeEnvironmentDialog from "@/features/dialogs/UnsafeEnvironmentDialog.vue";
import ImportCurlDialog from "@/features/dialogs/ImportCurlDialog.vue";
import AddCollectionDialog from "@/features/dialogs/AddCollectionDialog.vue";
import AddCollectionEndpointDialog from "@/features/dialogs/AddCollectionEndpointDialog.vue";
import AddToServiceDialog from "@/features/dialogs/AddToServiceDialog.vue";
...
const handleSelectEndpoint = (endpoint: any, knownServiceId?: string) => {
  ...
  if (existingTab) {
    ...
    const fullUrl =
      hasBaseUrl && !endpoint.url.includes("://") && !endpoint.url.includes("{{")
        ? `{{BASE_URL}}${path}`
        : endpoint.url;
    ...
    <ResizablePanelGroup direction="horizontal">
      ...
      <ResizablePanel :default-size="20" :min-size="15" :max-size="40">
        <ServiceSidebar
          ...
          @import-swagger="isSwaggerDialogOpen = true"
      ...
      <ResizablePanel :default-size="80">
        <RequestWorkspace
          ...
          label="Service"
          :on-new-request="
            servicesStore.services.length > 0 ? openEndpointDialog : undefined
          "
