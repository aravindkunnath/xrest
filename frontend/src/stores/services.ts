...
export const useServicesStore = defineStore("services", () => {
  ...
  const saveServices = async (commitMessage?: string) => {
    try {
      const updated = await serviceManager.saveServices(
        services.value,
        commitMessage,
      );
      if (updated) {
  ...
  const addService = async (service: Service, commitMessage?: string) => {
    try {
      const updated = await serviceManager.addService(
        services.value,
        service,
        commitMessage,
      );
  ...
  const updateService = async (
    ...
    try {
      const updated = await serviceManager.updateService(
        services.value,
        index,
        service,
        commitMessage,
  ...
  const importCurl = async (
    ...
    try {
      const updatedService = await serviceManager.importCurl(
        serviceId,
        curlCommand,
      );
      if (updatedService) {
  ...
  const importSwagger = async (
    ...
    try {
      const service = await serviceManager.importSwagger(name, filePath);
      if (service) {
        await loadServices();
        return service;
      }
      ...
      toast.error("Swagger Import Failed", {
        description: String(error),
      });
    }
    return null;
