...
const formattedCode = computed(() => {
  ...
  const lang = (props.language || "json").toLowerCase();
  if (lang.includes("json")) {
    try {
      const parsed = JSON.parse(props.code);
      return JSON.stringify(parsed, null, 2);
    } catch (e) {
      // Return raw code if not valid JSON
    }
  }
  return props.code;
});
...
<template>
  <div :class="cn('w-full h-full min-h-[120px]', props.class)">
    <CodeMirrorEditor
      :model-value="formattedCode"
      :read-only="true"
